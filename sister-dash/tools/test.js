...
try {
  ...
  // Shooting: every character shoots while big, each with their own shot and limits
  ...
  for (const kind of ['older', 'younger', 'grownup', 'firewoman']) {
    ...
    const fs = Game.scene;
    const pl = fs.player;
    const cfg = SHOTS[SHOT_OF[kind]];
    check(pl.cfg.shot === SHOT_OF[kind], `${kind} shoots ${cfg.name}`);
    ...
    check(fs.shots.length <= cfg.max, `${kind}: at most ${cfg.max} on screen (${fs.shots.length})`);
    step(400);
    check(fs.shots.length === 0, `${kind}: shots end (hit a wall, leave the screen or come back)`);
    // defeats an enemy that can't be stomped, for +1 coin
    const plant = fs.enemies.find((e) => e instanceof SpikyPlant && !e.killed);
    plant.active = true;
    fs.camX = plant.x - 300;
    pl.update = function () {};
    const coins = fs.coins;
    fs.shots = [new Shot(SHOT_OF[kind], plant.x - 30, plant.y + plant.h / 2, 1, false)];
    if (SHOT_OF[kind] === 'fireball') fs.shots[0].vy = 0;
    for (let i = 0; i < 30; i++) fs.updateShots(STEP);
    const gone = plant.killed || plant.trapped;
    check(gone && fs.coins === coins + 1, `${kind}: shot defeats a spiky plant, +1 coin`);
    if (SHOT_OF[kind] === 'bubble') {
      check(plant.trapped, 'bubble traps the enemy');
      step(200);
      check(plant.killed, 'trapped enemy floats off the screen');
    }
    delete pl.update;
  }
  // boomerang flies out about 5 tiles and comes back to be caught
  {
    Game.character = 'grownup';
    Game.setScene(new PlayScene(0));
    const bs = Game.scene;
    bs.player.setBig(true); bs.player.growT = 0;
    bs.player.update = function () {};
    keys.pressed.add('fire'); step(1);
    const b = bs.shots[0];
    let far = 0;
    for (let i = 0; i < 120 && bs.shots.length; i++) { step(1); far = Math.max(far, Math.abs(b.x + b.w / 2 - bs.player.cx)); }
    check(bs.shots.length === 0 && far > 4 * TILE && far < 7 * TILE, `boomerang: out ${Math.round(far / TILE * 10) / 10} tiles and caught`);
    keys.pressed.add('fire'); step(1);
    check(bs.shots.length === 1, 'boomerang: can throw again once caught');
  ...
  // underwater shots: half speed, straight line
  {
    const land = new Shot('fireball', 100, 100, 1, false);
    const sea = new Shot('fireball', 100, 100, 1, true);
    check(Math.abs(sea.vx) === Math.abs(land.vx) / 2 && sea.vy === 0, 'underwater shots move at half speed in a straight line');
  }
...
function reach(li, kind, big, breakTile) {
  ...
  const key = (p) => Math.round(p.x / bucket) + ',' + Math.round(p.y / (level.underwater ? bucket : 1));
  ...
  while (queue.size) {
    ...
    if (t0 >= finish) break;
    if (t0 > bestT.get(Math.round(sx / bucket) + ',' + Math.round(sy / (level.underwater ? bucket : 1)))) continue;
    ...
    for (const a of wet ? swimActions.concat(landActions.filter((x) => x.j < 999)) : landActions) {
      ...
      for (let f = 0; f < frames; f++) {
        ...
        else settled = (p.onGround && (a.j === 999 ? f > 18 && p.vx === 0 : airborne)) || (p.swimming && f > 12 && !wet);
        if (settled) {
...
const SPEC = [{ carrots: 4, coins: 120, cps: 3 }, { carrots: 5, coins: 160, cps: 3 }, { carrots: 6, coins: 200, cps: 3 }, { carrots: 5, coins: 200, cps: 3 }, { carrots: 4, coins: 220, cps: 3 }];
