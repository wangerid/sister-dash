...
const seg = {
  ...
  arena: (boss) => (m) => {
    ...
    const py = boss === 'octavia' ? 6 : 4; // underwater the rocks float higher
    for (const [c, w] of plats) for (let k = 0; k < w; k++) m.at(c + k, py, '#');
    m.at(a + 26, 2, 'F');
    m.x += 32;
  },
...
const BOSS_COINS = { bristle: 20, croakia: 30, frostbeak: 40, octavia: 45, sandclaw: 50 };
const BOSS_NAMES = { bristle: 'Big Bristle', croakia: 'Queen Croakia', frostbeak: 'Frostbeak', octavia: 'Admiral Octavia', sandclaw: 'King Sandclaw' };
...
for (const l of levels) {
  ...
  print(`${l.name}: width ${w}, coins total=${n('o') + n('?') + n('X') + BOSS_COINS[l.boss]} (o=${n('o')} ?=${n('?')} X=${n('X')} boss=${BOSS_COINS[l.boss]}), carrots=${n('C') + n('c')} (C=${n('C')} c=${n('c')}), checkpoints at ${cps.join(', ')}, h=${n('h')} f=${n('f')} w=${n('w')} s=${n('s')} z=${n('z')} v=${n('v')} t=${n('t')} r=${n('r')} j=${n('j')} p=${n('p')} e=${n('e')} vents=${n('a')}`);
}
