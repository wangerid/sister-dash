...
const SISTER_LOOKS = {
  ...
  firegirl: {
    top: '#ff8c2a', topDark: '#d9661a', legs: '#24345f', shoe: '#5a2a1a',
    hair: '#7b4524', hairDark: '#552e16', skin: '#f6d0ab',
    legLen: 10, bodyH: 11, headR: 6.3, style: 'long',
  },
};
...
function drawSister(ctx, kind, cx, bottom, scale, facing, pose, phase, t) {
  ...
  if (L.style === 'ponytail') {
    ...
    // green jacket, open at the front over a white shirt
    ctx.fillStyle = L.top;
...
// Soft orange glow behind the Fire Girl while her fireball power is active.
function drawFireGlow(ctx, cx, cy, r, t) {
  const g = ctx.createRadialGradient(cx, cy, 2, cx, cy, r);
  g.addColorStop(0, `rgba(255, 150, 40, ${0.38 + Math.sin(t * 6) * 0.08})`);
...
function drawHUD(ctx, scene) {
  ...
  ctx.globalAlpha = 1;
  if (scene.player.cfg.fireballs) {
    ctx.globalAlpha = scene.player.canShoot ? 1 : 0.25;
    drawFlame(ctx, x + 32, 24, 1.3, scene.player.canShoot ? Game.time : 0);
    ctx.globalAlpha = 1;
  }
