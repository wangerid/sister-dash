...
// Shiny crystal coins in the caves are worth 5 coins.
const CRYSTAL_VALUE = 5;

class CrystalCoin extends Coin {
  constructor(tx, ty) {
    super(tx, ty);
    this.value = CRYSTAL_VALUE;
  }

  draw(ctx, t) {
    drawCrystalCoin(ctx, this.x + 9, this.y + 10 + Math.sin(t * 3 + this.phase) * 1.5, t + this.phase);
  }
}
...
class Particles {
  ...
  smoke(x, y) {
    for (let i = 0; i < 10; i++) {
      ...
      this.list.push({ type: 'smoke', x, y, vx: Math.cos(a) * 40, vy: Math.sin(a) * 30 - 40, life: 0.8, max: 0.8, size: 7 + (i % 3) * 2 });
    }
  }
  ...
  draw(ctx) {
    for (const p of this.list) {
      ...
      if (p.type === 'puff') {
        ...
        circle(ctx, p.x, p.y, p.size * (1.6 - k * 0.8));
      } else if (p.type === 'star') {
