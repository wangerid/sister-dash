...
class Boss {
  ...
  // Any character's shot: 3 hits = 1 hit point.
  shotHit(scene) {
...
// Stalactites hang from the cave ceiling and shake before they drop.
class Drop {
  constructor(arena, x, kind, warn = 0.9) {
    this.kind = kind;
    this.w = kind === 'snowball' ? 22 : kind === 'stalactite' ? 16 : 12;
    this.h = kind === 'snowball' ? 22 : 26;
    this.x = x - this.w / 2;
    this.groundY = arena.surfaceBelow(x, kind === 'stalactite' ? arena.ceilingY : arena.floorY - 16 * TILE);
    this.y = kind === 'stalactite' ? arena.ceilingY : this.groundY - 520;
    this.warn = warn;
    this.maxWarn = warn;
    this.state = 'warn';
    this.vy = 0;
  ...
  update(dt, arena, scene) {
    if (this.state === 'warn') {
      this.warn -= dt;
      if (this.warn <= 0) { this.state = 'fall'; this.vy = this.kind === 'stalactite' ? 300 : 950; }
    } else if (this.state === 'fall') {
      if (this.kind === 'stalactite') this.vy = Math.min(this.vy + 1800 * dt, 900);
      this.y += this.vy * dt;
  ...
  draw(ctx, t) {
    ...
    if (this.kind === 'stalactite') { drawDropStalactite(ctx, this, t); return; }
...
class Lob {
  ...
  draw(ctx, t) {
    ...
    if (this.kind === 'rock') drawLobRock(ctx, this, t);
    else drawSpine(ctx, this, t);
  }
}
...
class BossArena {
  ...
  constructor(level, kx, ky, kind) {
    ...
    // underwater arenas have walls all the way up to the surface
    this.maxWallRows = level.underwater ? ky - Math.floor(level.surfaceY / TILE) + 1 : WALL_ROWS;
    this.won = false;
    // the cave arena has a rock ceiling that stalactites fall from
    this.ceilingY = 0;
    const midCol = Math.floor((kx + ARENA_TILES / 2));
    for (let ty = 0; ty < this.wallBottomRow; ty++) { if (level.isSolid(midCol, ty)) this.ceilingY = (ty + 1) * TILE; }
    this.reset();
  }
  ...
  reset(scene) {
    ...
    this.introT = 0;
    this.wind = 0;
    this.ink = null; // { side: -1 | 1, t } darkens half the arena
    this.rain = null; // { side: -1 | 1, t } heavy rain on half the arena
  ...
  setWallRows(n) {
    for (let i = 0; i < this.maxWallRows; i++) {
      const ch = i < n ? '&' : '.';
  ...
  // (thin clouds included: jumping up would carry the player through onto the cloud instead),
  ...
  freeSpot(x, hw) {
    const blocked = (cx) => {
      for (let tx = Math.floor((cx - hw) / TILE); tx <= Math.floor((cx + hw) / TILE); tx++) {
        for (let ty = this.wallBottomRow - 4; ty <= this.wallBottomRow; ty++) if (this.level.isFloor(tx, ty) && tx > this.leftCol && tx < this.rightCol) return true;
      }
      return false;
    };
  ...
  update(dt, scene) {
    ...
    if (this.state === 'waiting') {
      if (!this.won && p.state === 'play' && p.cx > (this.kx + 2) * TILE) {
        ...
        this.wantWalls = this.maxWallRows;
    ...
    if (this.ink && (this.ink.t -= dt) <= 0) this.ink = null;
    if (this.rain && (this.rain.t -= dt) <= 0) this.rain = null;
  ...
  // Is x inside the rainy half of the arena?
  inRain(x) {
    if (!this.rain) return false;
    const mid = (this.left + this.right) / 2;
    return this.rain.side < 0 ? x < mid : x >= mid;
  }

  // Is x inside the inked half of the arena?
  inInk(x) {
    if (!this.ink) return false;
    const mid = (this.left + this.right) / 2;
    return this.ink.side < 0 ? x < mid : x >= mid;
  }
  ...
  // Drawn over everything (including the player) so the ink really hides that half.
  drawOverlay(ctx, t) {
    if (this.ink) drawInk(ctx, this, t);
    if (this.rain) drawDownpour(ctx, this, t);
    if (this.wind && this.kind === 'grumble') drawGust(ctx, this, t);
  }
}
