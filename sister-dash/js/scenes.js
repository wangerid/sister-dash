...
class TitleScene {
  ...
  render(ctx) {
    ...
    drawSister(ctx, 'firewoman', 430, 440, 2.4, 1, 'run', phase + 2.1, this.t);
...
class CharacterSelectScene {
  constructor() {
    this.t = 0;
    this.kinds = ['older', 'younger', 'grownup', 'firewoman'];
  ...
  render(ctx) {
    ...
    const tint = { older: 'rgba(59,130,230,0.18)', younger: 'rgba(255,111,174,0.2)', grownup: 'rgba(63,145,80,0.2)', firewoman: 'rgba(255,140,42,0.22)' };
    for (let i = 0; i < this.kinds.length; i++) {
      ...
      ctx.fill();
      if (on) drawShotGlow(ctx, cx, y + 140, 80, this.t, SHOTS[cfg.shot].color);
      ...
      drawText(ctx, `Shoots ${SHOTS[cfg.shot].name}`, cx, y + 310, 14, '#7a7f95', 'center', null);
    }
...
// Difficulty label per level, in level order (easiest to hardest).
const LEVEL_INFO = [
  { difficulty: 'Easy' },
...
class LevelSelectScene {
  ...
  // Cards are laid out at 210×294 and drawn scaled down so five fit across.
  cardRect(i) {
    const k = LevelSelectScene.CARD_SCALE;
    return { x: 13 + i * 134, y: 110, w: 210 * k, h: 294 * k };
  }
  ...
  render(ctx) {
    ...
    // draw the selected card last, a little bigger, so it's easy to read
    const order = LEVELS.map((_, i) => i).filter((i) => i !== this.sel).concat([this.sel]);
    for (const i of order) {
      const sr = this.cardRect(i);
      const def = LEVELS[i];
      const rec = SaveData.getLevel(i);
      const on = i === this.sel;
      const k = LevelSelectScene.CARD_SCALE * (on ? 1.25 : 1);
      const cx = Math.max(210 * k / 2 + 6, Math.min(VIEW_W - 210 * k / 2 - 6, sr.x + sr.w / 2));
      ctx.save();
      ctx.translate(cx - (210 * k) / 2, sr.y + (on ? -22 : 0));
      ctx.scale(k, k);
      const r = { x: 0, y: 0, w: 210, h: 294 };
      const y = 0;
    ...
    const nm = SISTERS[Game.character].name;
    const who = Game.character === 'firewoman' ? `the ${nm}` : `the ${nm.toLowerCase()}`;
    drawText(ctx, `Playing as ${who}`, VIEW_W / 2, 486, 16, '#ffffff');
    drawHint(ctx, '← → to choose  •  Enter / Click to play  •  Esc: change character');
  }
}

LevelSelectScene.CARD_SCALE = 128 / 210;
...
class PlayScene {
  ...
  enter() {
    ...
    for (const s of this.level.spawns) {
      switch (s.type) {
        ...
        case '*': this.coinList.push(new CrystalCoin(s.tx, s.ty)); break;
    ...
    if (this.flag && this.level.themeName === 'clouds') this.flag.rainbow = true;
  ...
  update(dt) {
    ...
    p.airPaused = !!(ar && ar.locked);
    p.inkSlow = !!(ar && ar.inInk(p.cx));
    p.rainSlow = !!(ar && ar.inRain(p.cx));
    // Grumblecloud's gust: standing still on a platform lets her hold on
    if (ar && ar.kind === 'grumble' && ar.wind && p.onGround && p.bottom < ar.floorY - 4 && !Input.isDown('left') && !Input.isDown('right')) p.push -= ar.wind;
    if (ar && ar.kind === 'octavia' && ar.state === 'fight') p.push += Math.sign((ar.left + ar.right) / 2 - p.cx) * 35;
    ...
    for (const e of this.enemies) {
      ...
      if (e.trapped) {
        e.t += dt;
        e.y -= 160 * dt;
        e.x += Math.sin(e.t * 3) * 20 * dt;
        if (e.y + e.h < -40) e.killed = true;
        continue;
      }
      if (e.spinning) {
        // hit by a boomerang: spins away off the screen
        e.spinA += 14 * dt;
        e.svy += 1200 * dt;
        e.x += e.svx * dt;
        e.y += e.svy * dt;
        if (e.y > lvl.height + 60) e.killed = true;
        continue;
      }
      if (!e.active && e.shouldActivate(this.camX)) e.active = true;
      if (e.active) e.update(dt, lvl, p, this.camX, this);
  ...
  updateShots(dt) {
    const p = this.player;
    const kind = p.cfg.shot;
    if (this.shotCooldown > 0) this.shotCooldown -= dt;
    if (p.canShoot && Input.pressed('fire') && this.shotCooldown <= 0 && this.shots.length < SHOT.max) {
      const hx = p.facing > 0 ? p.x + p.w + 4 : p.x - 4;
      const hy = p.y + p.h * 0.4;
      this.shots.push(new Shot(kind, hx, hy, p.facing, this.level.inWater(hx, hy)));
    ...
    for (const s of this.shots) {
      s.update(dt, this.level, this.camX);
      if (s.done) continue;
      if (boss && boss.alive && overlaps(s.rect, boss.hitbox)) {
        boss.shotHit(this); // 3 shot hits = 1 hit point
        s.done = true;
      }
      for (const e of this.enemies) {
        if (s.done) break;
        if (e.killed || e.trapped || e.spinning || !e.active) continue;
        const hb = e.hitbox;
        if (!hb || !overlaps(s.rect, hb)) continue;
        // shots defeat any enemy, even the ones that can't be stomped
        this.shotDefeat(e, kind, s.dir);
        this.addCoin(1);
        s.done = true;
      }
      if (s.done) this.particles.poof(s.x + s.w / 2, s.y + s.h / 2);
    }
    this.shots = this.shots.filter((s) => !s.done);
  }

  // Each character's shot finishes an enemy off in its own way.
  shotDefeat(e, kind, dir) {
    const effect = SHOTS[kind].defeat;
    const cx = e.x + e.w / 2, cy = e.y + e.h / 2;
    if (effect === 'trap') return this.trap(e);
    if (effect === 'spin') {
      e.spinning = true;
      e.spinA = 0;
      e.svx = dir * 220;
    ...
    e.killed = true;
    if (effect === 'sparkles') this.particles.sparkle(cx, cy, 16, '#ffe066');
    else this.particles.smoke(cx, cy);
    Sound.play('stomp');
  }

  // Bubbles trap an enemy, which then floats away off the screen.
  trap(e) {
    e.trapped = true;
    Sound.play('appear');
  }
  ...
  renderWorld(ctx) {
    ...
    if (lvl.themeName === 'caves') drawLadder(ctx, this.start.cx - 30, 0, this.start.bottom);
    drawTiles(ctx, lvl, this.camX, t);
    drawMovers(ctx, lvl, t);
    ...
    for (const e of this.enemies) {
      if (!e.killed && (vis(e.x) || e.warning)) {
        if (e.spinning) {
          ctx.save();
          ctx.translate(e.x + e.w / 2, e.y + e.h / 2);
          ctx.rotate(e.spinA);
          ctx.translate(-(e.x + e.w / 2), -(e.y + e.h / 2));
          e.draw(ctx, t, this.camX);
          ctx.restore();
          continue;
        }
        e.draw(ctx, t, this.camX);
        if (e.trapped) drawTrapBubble(ctx, e, t);
      }
    }
    if (this.arena) this.arena.draw(ctx, t);
    for (const pc of this.popCoins) pc.draw(ctx, t);
    for (const sh of this.shots) sh.draw(ctx, t);
    this.player.draw(ctx, t);
    drawQuicksand(ctx, lvl, this.camX, t); // drawn over the player so she sinks into it
    this.particles.draw(ctx);
    if (lvl.dark.length) drawDarkness(ctx, lvl, this.player, this.camX);
    if (this.arena) this.arena.drawOverlay(ctx, t);
    ctx.restore();
    if (lvl.underwater) drawWaterTint(ctx, lvl, this.camY, t);
  }
