// bosses.js – the seven bosses and their attacks (engine in boss.js)
...
// A giant mole with a miner's helmet. Dives underground and a dirt mound chases the player;
// he bursts up where it stops and gets his head stuck in the dirt (dizzy 2 s). Also tosses rocks,
// and in phase 2 shakes stalactites loose from the ceiling.

class DiggerDuke extends Boss {
  constructor(arena) {
    super(arena, BOSS_TYPES.duke);
    this.x = arena.right - 160 - this.w;
    this.restY = arena.floorY - this.h;
    this.y = arena.floorY + 10;
    this.moundX = null;
  }

  get underground() { return this.state === 'dive' || this.state === 'tunnel' || this.state === 'rumble'; }
  get harmful() { return this.alive && this.state !== 'intro' && !this.weak && !this.underground; }

  // bursts up out of the ground
  intro(dt, k, scene) {
    this.y = this.arena.floorY + 10 - (this.h + 10) * Math.min(1, k / 0.5);
    if (k > 0.5 && !this.popped) { this.popped = true; scene.shake = 0.4; Sound.play('crash'); scene.particles.debris(this.cx, this.arena.floorY - 10); }
  }

  nextAttack(p, scene) {
    const pattern = this.phase === 1 ? ['dig', 'rocks'] : ['dig', 'rocks', 'dig', 'shake'];
    const attack = pattern[this.attackIdx % pattern.length];
    this.attackIdx++;
    this.facePlayer(p);
    if (attack === 'dig') { this.setState('dive', 0.5); Sound.play('rumble'); }
    else if (attack === 'rocks') { this.setState('scoop', 0.8); Sound.play('stamp'); }
    else { this.setState('stamp', 0.8); Sound.play('stamp'); }
  }
  ...
  think(dt, scene, p) {
    ...
    switch (this.state) {
      case 'dive':
        this.t -= dt;
        this.y = Math.min(a.floorY + 10, this.y + 200 * dt);
        if (this.t <= 0) { this.moundX = this.cx; this.setState('tunnel', this.phase === 1 ? 2.2 : 1.8); }
        break;
      case 'tunnel': {
        // the dirt mound follows the player along the floor
        this.t -= dt;
        this.y = a.floorY + 10; // out of sight underground
        const sp = this.phase === 1 ? 150 : 190;
        const d = p.cx - this.moundX;
        this.moundX += Math.sign(d) * Math.min(Math.abs(d), sp * dt);
        if (Math.random() < 0.3) scene.particles.dust(this.moundX, a.floorY);
        if (this.t <= 0 || Math.abs(d) < 6) {
          this.moundX = a.freeSpot(this.moundX, this.w / 2); // never under a ledge
          this.setState('rumble', 0.6);
        }
        break;
      }
      case 'rumble':
        this.t -= dt;
        scene.shake = Math.max(scene.shake, 0.05);
        if (this.t <= 0) {
          this.x = this.moundX - this.w / 2;
          this.setState('pop', 0.25);
          Sound.play('crash');
          scene.particles.debris(this.moundX, a.floorY - 10);
        }
        break;
      case 'pop':
        this.t -= dt;
        this.y = a.floorY - this.h * (1 - Math.max(0, this.t) / 0.25);
        if (this.t <= 0) { this.y = a.floorY - this.h; this.moundX = null; this.goWeak(scene); } // head stuck in the dirt
        break;
      case 'scoop':
        this.t -= dt;
        if (this.t <= 0) {
          for (const off of [0, this.phase === 1 ? 130 : -130]) {
            a.hazards.push(new Lob(a, this.cx, this.y + 10, a.clampX(p.cx + off), 1.3, 'rock'));
          }
          this.setState('scoopWait', 1.4);
        }
        break;
      case 'scoopWait':
        this.t -= dt;
      ...
      case 'stamp':
        ...
        if (this.t <= 0) {
          for (const x of a.dropSpots(3 + Math.floor(Math.random() * 2), p)) a.hazards.push(new Drop(a, x, 'stalactite'));
          this.setState('stampWait', 1.6);
        }
        break;
      case 'stampWait':
        this.t -= dt;
  ...
  draw(ctx, t) { drawDuke(ctx, this, t); }
}
...
// A huge angry storm cloud floating above the arena's cloud floor. After a lightning bolt he runs
// out of energy and sinks down low, pale and puffy (stomp him then). Also a downpour on one half,
// in phase 2 a gust toward the wall, and in phase 3 a thunder storm of three bolts.

class Bolt {
  constructor(arena, x, warn) {
    this.kind = 'bolt';
    this.x = x;
    this.top = arena.floorY - 12 * TILE;
  ...
  get shadowX() { return this.state === 'warn' || this.state === 'strike' ? this.x : null; }
  get hurts() { return this.state === 'strike'; }
  get rect() { return { x: this.x - 12, y: this.top, w: 24, h: this.floorY - this.top }; }

  update(dt, arena, scene) {
    if (this.state === 'warn') {
      this.warn -= dt;
      if (this.warn <= 0) { this.state = 'strike'; Sound.play('zap'); scene.shake = Math.max(scene.shake, 0.2); }
    } else {
      this.strike -= dt;
      if (this.strike <= 0) this.done = true;
    }
  }

  draw(ctx, t) { drawBolt(ctx, this, t); }
}

class Grumblecloud extends Boss {
  constructor(arena) {
    super(arena, BOSS_TYPES.grumble);
    // high enough to be out of reach even from the thin clouds
    this.highY = arena.floorY - 360;
    this.x = (arena.left + arena.right) / 2 - this.w / 2;
    this.y = arena.floorY - 700;
    this.bolts = [];
  }

  get lowY() { return this.arena.floorY - this.h - 4; }
  get harmful() { return this.alive && this.state !== 'intro' && !this.weak && this.state !== 'sink'; }

  intro(dt, k) {
    this.y = this.arena.floorY - 700 + (this.highY - (this.arena.floorY - 700)) * Math.min(1, k / 0.8);
  }

  // wobbles and puffs back up for a moment before rising again
  recover(scene) {
    this.setState('wake', 0.4);
  }

  rest() {
    if (this.y > this.highY + 20) { this.setState('rise'); return; }
    super.rest();
  }

  nextAttack(p, scene) {
    if (this.y > this.highY + 20) { this.setState('rise'); return; } // always back up high before attacking
    const pattern = this.phase === 1 ? ['bolt', 'rain'] : this.phase === 2 ? ['bolt', 'rain', 'gust'] : ['bolt', 'storm', 'gust', 'rain'];
    const attack = pattern[this.attackIdx % pattern.length];
    this.attackIdx++;
    const a = this.arena;
    if (attack === 'bolt') {
      const x = a.freeSpot(p.cx, this.w / 2); // where he can sink down afterwards
      this.bolts = [new Bolt(a, x, 1)];
      a.hazards.push(this.bolts[0]);
      this.targetX = x - this.w / 2;
      this.setState('crackle');
      Sound.play('appear');
    } else if (attack === 'storm') {
      const xs = [p.cx - 150, p.cx, p.cx + 150].map((x) => a.clampX(x, 20)).sort((m, n) => m - n);
      this.bolts = xs.map((x, i) => new Bolt(a, x, 1 + i * 0.35)); // left to right
      for (const b of this.bolts) a.hazards.push(b);
      this.setState('storming', 2.2);
      Sound.play('roar');
    } else if (attack === 'rain') {
      this.setState('frown', 0.8);
    } else {
      this.facePlayer(p);
      this.setState('puff', 0.7);
      Sound.play('roar');
    }
  }
  ...
  think(dt, scene, p) {
    ...
    switch (this.state) {
      case 'crackle':
        this.x += Math.sign(this.targetX - this.x) * Math.min(Math.abs(this.targetX - this.x), 300 * dt);
        if (this.bolts.every((b) => b.done)) this.setState('sink'); // out of energy
        break;
      case 'sink':
        this.y = Math.min(this.lowY, this.y + 400 * dt);
        if (this.y >= this.lowY) this.goWeak(scene);
        break;
      case 'wake':
        this.t -= dt;
        ...
        break;
      case 'rise':
        this.y = Math.max(this.highY, this.y - 250 * dt);
        if (this.y <= this.highY) super.rest();
        break;
      case 'storming':
        this.t -= dt;
        if (this.t <= 0) this.rest();
      ...
      case 'frown':
        this.t -= dt;
        if (this.t <= 0) {
          a.rain = { side: p.cx < (a.left + a.right) / 2 ? -1 : 1, t: 3 };
          this.rest();
        }
        break;
      case 'puff':
        this.t -= dt;
        if (this.t <= 0) {
          a.wind = (p.cx < this.cx ? -1 : 1) * 150; // toward the wall on the player's side
          this.setState('gusting', 2);
        }
        break;
      case 'gusting':
        this.t -= dt;
    ...
    // drift above the player while up high
    if (this.state === 'idle' || this.state === 'frown' || this.state === 'puff' || this.state === 'gusting') {
      const tx = a.clampX(p.cx, this.w / 2 + 4) - this.w / 2;
      this.x += Math.sign(tx - this.x) * Math.min(Math.abs(tx - this.x), 90 * dt);
    }
  }

  // rains himself out into a tiny smiling cloud that floats away
  flee(dt) {
    this.y -= 60 * dt;
    this.x += 40 * dt;
    if (this.y < this.arena.floorY - 700) this.setState('gone');
  }

  draw(ctx, t) { drawGrumblecloud(ctx, this, t); }
}
...
const BOSS_TYPES = {
  ...
  duke: {
    cls: DiggerDuke, name: 'Digger Duke', blurb: 'a giant mole with a miner\'s helmet and lamp',
    hp: 3, w: 90, h: 64, coins: 25, weakTime: 2, rest: [1, 0.7], phaseOf: (hp) => (hp >= 2 ? 1 : 2),
  },
  croakia: {
  ...
  grumble: {
    cls: Grumblecloud, name: 'Grumblecloud', blurb: 'a huge angry storm cloud',
    hp: 4, w: 130, h: 70, coins: 35, weakTime: 2, rest: [1, 0.8, 0.6], phaseOf: (hp) => (hp >= 3 ? 1 : hp === 2 ? 2 : 3),
  },
  frostbeak: {
