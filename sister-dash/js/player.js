...
const SISTERS = {
  ...
  older: { name: 'Older sister', speed: 231, jump: 1.0, smallH: 32, w: 20, perk: 'Runs a little faster', look: 'Blonde ponytail' },
  younger: { name: 'Younger sister', speed: 210, jump: Math.sqrt(1.1), smallH: 28, w: 20, perk: 'Jumps a little higher', look: 'Blonde pigtails' },
  grownup: { name: 'Grown-up', speed: 210, jump: Math.sqrt(0.95), smallH: 40, w: 22, breaksCracked: true, perk: 'Smashes cracked boxes', look: 'Even when small' },
  firegirl: { name: 'Fire Girl', speed: 210, jump: 1.0, smallH: 32, w: 20, fireballs: true, perk: 'Throws fireballs', look: 'When big (X / Shift / J)' },
};
...
class Player {
  constructor(kind, cx, bottom) {
    ...
    this.push = 0; // external horizontal push in px/s (icy wind)
  ...
  get canShoot() { return !!this.cfg.fireballs && this.big && this.state === 'play'; }
  ...
  update(dt, level, scene) {
    ...
    if (this.state === 'dead') {
      this.vy = Math.min(this.vy + PHYS.gravity * dt, PHYS.fallMax);
      this.y += this.vy * dt;
      return;
    }
    ...
    const maxSpeed = this.cfg.speed * (this.inSand ? SAND.speed : 1);
    ...
    if (this.jumpBuffer > 0 && this.coyote > 0) {
      this.vy = -PHYS.jumpV * this.cfg.jump * (this.inSand ? SAND.jump : 1);
      this.onGround = false;
      this.coyote = 0;
      this.jumpBuffer = 0;
      Sound.play('jump');
    ...
    let g = PHYS.gravity;
    if (this.vy < 0 && !(control && Input.isDown('jump'))) g *= PHYS.cutGravity;
    this.vy = Math.min(this.vy + g * dt, PHYS.fallMax);
    if (this.inSand && this.vy > SAND.sink) this.vy = SAND.sink;

    this.prevBottom = this.y + this.h;
    if (level.moveX(this, (this.vx + this.push) * dt)) this.vx = 0;
  ...
  get pose() {
    ...
    if (this.throwT > 0 && this.onGround) return 'throw';
  ...
  draw(ctx, t) {
    if (this.canShoot) drawFireGlow(ctx, this.cx, this.y + this.h * 0.55, this.h * 0.85, t);
