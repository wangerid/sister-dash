...
class LevelSelectScene {
  ...
  cardRect(i) {
    ...
    return { x: 16 + i * 188, y: 104, w: 210 * k, h: 294 * k };
  }
  ...
  render(ctx) {
    ...
    for (let i = 0; i < LEVELS.length; i++) {
      ...
      const on = i === this.sel;
      ctx.save();
      ctx.translate(sr.x, sr.y + (on ? -8 : 0));
      ctx.scale(LevelSelectScene.CARD_SCALE, LevelSelectScene.CARD_SCALE);
...
LevelSelectScene.CARD_SCALE = 176 / 210;
...
class PlayScene {
  ...
  updateShots(dt) {
    ...
    if (p.canShoot && Input.pressed('fire') && this.shotCooldown <= 0 && this.shots.length < cfg.max) {
      ...
      this.shotCooldown = cfg.cooldown;
      p.throwT = 0.18;
      Sound.play('fire');
    }
    const boss = this.arena && this.arena.boss;
    for (const s of this.shots) {
      s.update(dt, this.level, this.camX, p);
      if (s.done) continue;
      if (boss && boss.alive && !s.hit.has(boss) && overlaps(s.rect, boss.hitbox)) {
        boss.shotHit(this); // 3 shot hits = 1 hit point
        s.hit.add(boss);
        if (!s.pierces) s.done = true;
      }
      for (const e of this.enemies) {
        if (s.done) break;
        if (e.killed || e.trapped || !e.active || s.hit.has(e)) continue;
        const hb = e.hitbox;
        if (!hb || !overlaps(s.rect, hb)) continue;
        // shots defeat any enemy, even the ones that can't be stomped
        if (kind === 'bubble') this.trap(e);
        else this.defeat(e);
        this.addCoin(1);
        s.hit.add(e);
        if (!s.pierces) s.done = true;
      }
      if (s.done && kind !== 'boomerang') this.particles.poof(s.x + s.w / 2, s.y + s.h / 2);
    }
  ...
  checkCollisions() {
    ...
    for (const e of this.enemies) {
      if (e.killed || e.trapped || !e.active) continue;
      const hb = e.hitbox;
